# Dashboard: Métricas de Gastos/Margen + Rediseño Visual — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Agregar métricas de Gastos del período y Margen neto (con un nuevo gráfico Ingresos vs Gastos) al Dashboard, y retonear todo el componente a los tokens del sistema de diseño shadcn (`bg-card`, `text-foreground`, `bg-primary`, etc.) ya usado en `Layout.jsx`/`Topbar.jsx`/`Sidebar.jsx`.

**Architecture:** Un único componente de página, `frontend/src/pages/Dashboard.jsx`. Se reutiliza el patrón existente de `useQuery` + filtrado client-side por rango de fecha, añadiendo una query hermana para `GET /libro-caja` (ya expuesta por el backend, sin cambios ahí). El rediseño visual se aplica en la misma pasada porque casi todas las líneas del archivo cambian de clase de color — separarlo en dos tareas forzaría un commit intermedio con colores mixtos sin valor de revisión propio.

**Tech Stack:** React 18, Recharts 3.9, TanStack Query 5, Tailwind 3.4 con tokens CSS-var (`frontend/src/index.css`, `frontend/tailwind.config.js`), lucide-react.

## Global Constraints

- No existe test runner de frontend en este proyecto — la verificación es `npm run lint`, `npm run build`, e inspección manual en el navegador (dev server). No fabricar resultados de "tests" que no existen.
- Los colores categóricos/semánticos existentes se mantienen sin cambio: `PALETA` (paleta de 8 colores para Top productos), `COLORES_METODO` (colores fijos por método de pago), el verde `#10b981` de la serie "pedidos" en el gráfico de Ingresos, y el rosa `#f43f5e`/verde `#10b981` del gráfico Cobrados vs Cancelados. Ninguno de estos deriva del color de marca — son códigos de color con significado propio.
- El color de marca (`--primary`, configurado por el negocio vía `BrandTheme`) se refleja en: el fondo del header de saludo, el tab activo (Día/Mes/Año), y el stroke/gradiente del gráfico principal de Ingresos.
- **Desviación deliberada del spec, documentada aquí:** el spec proponía un gradiente `from-primary to-accent` para el header. `--accent` en modo claro es un tono casi blanco (`189 45% 92%`) pensado para superficies de hover, no para fondos con texto blanco encima — usarlo como extremo de gradiente arriesga contraste ilegible (ver hallazgo ya corregido en la fase anterior sobre el uso de `--accent`). En su lugar, el header usa `bg-primary` sólido con `text-primary-foreground`, que por construcción del sistema de tokens garantiza contraste legible en ambos modos.
- Todo lo nuevo (cards de Gastos/Margen neto + gráfico Ingresos vs Gastos) se gatea por `puedeVerGastos = tiene(usuario, 'libro_caja', 'ver')`, igual patrón que `puedeVerVentas`/`puedeVerCaja` ya existentes. Si el usuario no tiene el permiso, el Dashboard se comporta exactamente igual que hoy.
- `GET /libro-caja` (`frontend/src/api/libroCaja.js:3-4` → backend `backend/src/modules/libro_caja/libro_caja.routes.js:9`) ya existe, ya está gateado por `libro_caja.ver`, y sin `sesion_caja_id` devuelve todos los movimientos de la sucursal con `tipo: 'ingreso'|'egreso'`, `monto`, `creado_en`. No se toca el backend.

---

### Task 1: Métricas de gastos/margen neto + rediseño visual completo de Dashboard.jsx

**Files:**
- Modify: `frontend/src/pages/Dashboard.jsx` (reescritura completa del archivo)

**Interfaces:**
- Consumes: `getLibroCaja` de `frontend/src/api/libroCaja.js` (ya existe, sin cambios — `api.get('/libro-caja', { params }).then(r => r.data.datos)`, llamado sin params devuelve todos los movimientos con `{ id, tipo, monto, creado_en, ... }`).
- Consumes: `tiene(usuario, modulo, accion)` helper ya definido en el propio archivo (línea 28 del original).
- No produce interfaces nuevas para otros archivos — este es el único archivo de la tarea.
...
Expected: sin errores nuevos atribuibles a `Dashboard.jsx` (0 errores en ese archivo).

- [ ] **Step 3: Verificar build**

Run: `cd frontend && npm run build`
Expected: build exitoso, sin errores de compilación/importación (confirma que `getLibroCaja`, los íconos `TrendingDown`/`PiggyBank` de `lucide-react`, y el nuevo JSX son válidos).

- [ ] **Step 4: Verificación manual en navegador**

Con el backend y `npm run dev` corriendo (el usuario los levanta manualmente — no iniciar servidores de fondo sin permiso):
1. Iniciar sesión con un usuario que tenga permiso `libro_caja.ver` (el rol Administrador semilla lo tiene, ver `bd/bd_codeculinary.sql` tabla `roles_permisos` rol 1).
2. Confirmar que aparecen las cards "Gastos del período" y "Margen neto", y el gráfico "Ingresos vs Gastos".
3. Cambiar de tab (Día/Mes/Año) y confirmar que las nuevas cifras cambian junto con las existentes.
4. Ir a Configuración, cambiar `color_primario` a un color muy distinto (p.ej. verde), volver al Dashboard y confirmar que el header y el stroke del gráfico de Ingresos cambian de color, en modo claro y oscuro.
5. (Opcional, si hay tiempo) Loguearse con un usuario sin `libro_caja.ver` y confirmar que el Dashboard se ve exactamente como antes de este cambio (sin las cards/gráfico nuevos, sin errores en consola).

Reportar en el resultado de la tarea si esta verificación manual se pudo ejecutar (requiere navegador, no disponible para subagentes sin acceso a browser) o si quedó pendiente para que el usuario la confirme.

- [ ] **Step 5: Commit**

```bash
git add frontend/src/pages/Dashboard.jsx
git commit -m "feat(dashboard): agregar métricas de gastos/margen neto y retonear a shadcn"
```
