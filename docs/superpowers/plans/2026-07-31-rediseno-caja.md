# Rediseño Visual: CajaPage Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Retonear `frontend/src/pages/caja/CajaPage.jsx` (página principal + los 4 modales que contiene) a los tokens shadcn ya definidos en el proyecto, sin cambiar layout, lógica, ni comportamiento — incluyendo la funcionalidad de reimpresión de tickets recién agregada.

**Architecture:** Reemplazo completo de un único archivo. Es un cambio puramente de clases Tailwind (colores) — cero cambios de JSX estructural, props, hooks, o llamadas a API.

**Tech Stack:** React 18, Tailwind 3.4 (tokens shadcn ya definidos en `frontend/src/index.css`/`frontend/tailwind.config.js`), `@tanstack/react-query`, `lucide-react`.

## Global Constraints

- No hay test runner de frontend en este proyecto — la verificación es `npm run lint` + `npm run build` (dentro de `frontend/`) + inspección manual (documentado en specs previos).
- Alcance: solo `frontend/src/pages/caja/CajaPage.jsx`. Ningún otro archivo.
- **Se mantienen sin cambio** (paleta categórica fija, mismo criterio que las StatCards del Dashboard — no son acciones de marca):
  - Las 3 `MetricCard` (Apertura=azul, Ventas=verde, Gastos=rojo) — objeto `colores` interno, sin cambios.
  - `diferenciaColor()` (verde=exacto, azul=sobrante, rojo=faltante) y toda su aplicación (resumen de sesión, `ModalCerrarCaja`, `ModalReporte`, tabla de historial).
  - El mini-grid de 3 métricas en las tarjetas móviles del historial (Apertura=azul, Ventas=verde) — extensión visual del mismo esquema que `MetricCard`; solo el tile "Cierre" (gris neutro, sin significado categórico) sí se retonea a `bg-muted`/`text-muted-foreground`/`text-foreground`.
  - Las columnas Ventas (verde) y Gastos (rojo) en la tabla de historial — mismo esquema categórico.
  - Los tiles Apertura/Ventas/Gastos dentro de `ModalCerrarCaja` — mismo esquema, sin cambios.
  - Chips/dots de método de pago: efectivo (verde), QR/transferencia (azul), incluida la nota "Ventas cobradas por QR" (púrpura) — categórico, no de marca.
  - Badges de estado "Caja Abierta"/"Abierta" (verde con punto pulsante) y "Disponible" (gris neutro → sí se retonea a `bg-muted`/`text-muted-foreground`, ver detalle en Task 1).
  - Avisos ámbar existentes — sin cambios, ya usan el patrón establecido.
  - El ícono de éxito verde (`CheckCircle2`) en `ModalReporte`.
  - La barra "Efectivo esperado en caja" (bajo las 3 MetricCard) y el aviso instructivo de `ModalAbrirCaja` ("Ingresa el monto...") — **decisión explícita:** se mantienen azules literales, por consistencia visual con las MetricCard que los rodean, en vez de tokenizarse a `bg-primary`.
- Botones de acción principal (Abrir Caja, Registrar gasto/Registrar, Cerrar reporte, pestaña activa del toggle) + focus rings → `bg-primary`/`text-primary-foreground`/`focus:ring-ring`.
- "Confirmar cierre" (botón sólido, cierra la sesión de caja) → `bg-destructive`/`text-destructive-foreground`.
- "Cerrar Caja" (botón outline) y mensajes de error de formulario → `text-destructive`.
- El ícono decorativo `Wallet` en `TarjetaCaja` (no es parte del esquema categórico de métricas, es un ícono genérico de identidad) → `bg-primary/10 text-primary`.

---

### Task 1: Retonear CajaPage.jsx completo

**Files:**
- Modify: `frontend/src/pages/caja/CajaPage.jsx` (reemplazo completo del archivo)

**Interfaces:**
- Sin cambios de props, exports, ni firmas de función. `CajaPage` (default export), y los componentes internos `TarjetaCaja`, `MetricCard`, `ModalAbrirCaja`, `ModalCerrarCaja`, `ModalGasto`, `ModalReporte` mantienen exactamente las mismas props y comportamiento.

- [ ] **Step 1: Reemplazar el contenido completo de CajaPage.jsx**

Reemplazar TODO el contenido de `frontend/src/pages/caja/CajaPage.jsx` con exactamente este código:
...
- Vista de grilla de cajas (disponible/abierta), apertura de caja, vista de detalle de sesión abierta, registro de gasto, cierre con conteo detallado y con monto total, historial de cierres (vista mobile apilada y vista desktop en tabla), reporte final con reimpresión de ticket — todo en modo claro y oscuro, con el color de marca (`color_primario`) configurado, verificando que los botones de acción (Abrir Caja, Registrar gasto/Registrar, Cerrar reporte, pestaña activa) y el ícono de `TarjetaCaja` reflejan el color de marca, mientras que las métricas Apertura/Ventas/Gastos y el indicador de Diferencia mantienen su paleta fija.

- [ ] **Step 4: Commit**

```bash
git add frontend/src/pages/caja/CajaPage.jsx
git commit -m "style(caja): retonear CajaPage a los tokens shadcn"
```
