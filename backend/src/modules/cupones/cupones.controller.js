...
async function validar(req, res, next) {
  try {
    ...
    let items = null;
    if (req.query.items) {
      try { items = JSON.parse(req.query.items); } catch { items = null; }
    }
    res.json({ ok: true, datos: await svc.validar(req.params.codigo, subtotal, req.query.cliente_id || null, items) });
  } catch (err) { next(err); }
}
